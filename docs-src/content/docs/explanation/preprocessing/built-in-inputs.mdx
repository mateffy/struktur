...
Lower-level helper that creates an artifact from a Buffer using the `providers` registry (a `Record<string, (buffer: Buffer) => Promise<Artifact>>`).

**Important:** `fileToArtifact` does **not** invoke the parser system — it uses the providers registry directly, which does not include the built-in PDF parser or any parsers configured via `config parsers add`. For PDF and other format support, use `parseInputToArtifacts` instead.
