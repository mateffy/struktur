...
description: Struktur is a TypeScript extraction engine that turns pre-parsed documents into validated JSON using LLMs.
---

Struktur is a TypeScript extraction engine that turns pre-parsed documents into validated, schema-typed JSON. It operates on JSON-based artifact DTOs, chunks them by token budgets, runs extraction strategies, validates results with Ajv, and merges or deduplicates outputs where needed.
