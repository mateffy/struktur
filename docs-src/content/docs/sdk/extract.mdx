...
| `fields` | One of `schema` or `fields` | [Fields shorthand](./fields) — comma-separated field definitions |
...
`fields` supports scalar types (`string`, `number`/`float`, `boolean`/`bool`, `integer`, `int`), enum sets (`status:enum{draft|live}`), and arrays (`tags:array{string}`). See the [Fields Shorthand](./fields) reference for the full syntax.

## See also

- [Fields Shorthand](./fields) — quick schema syntax reference
