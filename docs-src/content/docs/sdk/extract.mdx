...
| `fields` | One of `schema` or `fields` | [Fields shorthand](/docs/sdk/fields) — comma-separated field definitions |
...
`fields` supports scalar types (`string`, `number`/`float`, `boolean`/`bool`, `integer`, `int`), enum sets (`status:enum{draft|live}`), and arrays (`tags:array{string}`). See the [Fields Shorthand](/docs/sdk/fields) reference for the full syntax.

## See also

- [Fields Shorthand](/docs/sdk/fields) — quick schema syntax reference
- [Installation & Setup](/docs/cli/installation) — getting started
- [Strategy Factory Functions](/docs/explanation/strategies) — creating strategies
