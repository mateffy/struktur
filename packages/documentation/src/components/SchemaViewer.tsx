...
const getConstraints = (schema: JSONSchema): Constraint[] => {
  ...
  if (schema.minLength !== undefined) constraints.push({ label: 'minLen', value: schema.minLength });
  if (schema.maxLength !== undefined) constraints.push({ label: 'maxLen', value: schema.maxLength });
  if (schema.minItems !== undefined) constraints.push({ label: 'minItems', value: schema.minItems });
  if (schema.maxItems !== undefined) constraints.push({ label: 'maxItems', value: schema.maxItems });
  ...
  return (
    <div className="property-row">
      ...
      >
        ...
        {/* Type icon */}
        <span className="type-icon" style={{ 
          color: typeColor,
          display: 'flex',
          alignItems: 'center',
          marginTop: '1px'
        ...
        {/* Property name */}
        <span className="property-name" style={{
        ...
        {constraints.length > 0 && (
          <span className="constraints" style={{
            display: 'flex',
            gap: '4px',
            marginLeft: '4px',
          }}>
            {constraints.map((c) => (
              ...
              >
                {c.label}:{typeof c.value === 'string' && c.value.length > 10 
                  ? c.value.slice(0, 10) + '...' 
                  : c.value}
              </span>
            ))}
        ...
        {schema.default !== undefined && (
          <span className="default-value" style={{
            fontSize: '11px',
            color: '#7a9c5a',
            marginLeft: 'auto',
            fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace',
      ...
      {/* Enum values */}
      {schema.enum && (
        <div className="enum-values" style={{
          marginLeft: `${depth * 12 + 28}px`,
          padding: '2px 8px 6px',
          display: 'flex',
          flexWrap: 'wrap',
      ...
      {hasChildren && expanded && (
        <div className="property-children" style={{
          marginTop: '2px',
        }}>
...
const ArrayItems: React.FC<ArrayItemsProps> = ({ items, depth, defaultExpanded }) => {
  ...
  return (
    <div className="array-items-simple" style={{
      marginLeft: `${depth * 12 + 28}px`,
      padding: '4px 8px',
      display: 'flex',
      alignItems: 'center',
      ...
      <span style={{ color: getTypeColor(items.type) }}>
        {getTypeIcon(items)}
      </span>
      <span style={{
        fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace',
      ...
      {items.description && (
        <span style={{ fontSize: '11px', color: '#a0926f' }}>
          — {items.description}
        </span>
      )}
    </div>
      ...
      <div className="schema-body" style={{ maxHeight: '400px', overflow: 'auto' }}>
        ...
        {!schema.properties && !schema.items && (
          <div style={{
            ...
            <span style={{ color: getTypeColor(schema.type) }}>
              {getTypeIcon(schema)}
            </span>
            <span style={{
              fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace',
            ...
            {schema.description && (
              <span style={{ fontSize: '12px', color: '#7a5c3a' }}>
                — {schema.description}
              </span>
            )}
          </div>
