...
import { vsSource } from '@/lib/vs-source';
...
export const Route = createFileRoute('/vs/$')({
  ...
  head: ({ loaderData }) => {
    ...
    const description = 'Compare Struktur to alternatives for structured data extraction and document processing.';
    ...
    return {
      meta: [
        ...
        { property: "og:url", content: `https://struktur.sh/vs/${slugs?.join('/') ?? ''}` },
      ...
      links: [
        { rel: "canonical", href: `https://struktur.sh/vs/${slugs?.join('/') ?? ''}` },
      ],
    };
  },
});
...
const clientLoader = browserCollections.vs.createClientLoader({
  component(
      ...
      <DocsPage toc={toc}>
        ...
        <div className="flex flex-row gap-2 items-center border-b -mt-4 pb-6">
          ...
          <ViewOptions
            ...
            githubUrl={`https://github.com/${gitConfig.user}/${gitConfig.repo}/blob/${gitConfig.branch}/content/vs/${path}`}
          />
        </div>
        <DocsBody>
          <MDX
