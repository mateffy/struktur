...
export function ArtifactCharts({
	artifacts,
	chunkingSettings,
}: ArtifactChartsProps) {
			...
			artifact.contents.forEach((content) => {
				if (content.page !== undefined) {
					...
					const embeddedImages =
						content.media?.filter((m) => m.imageType === "embedded") || [];
					const screenshotImages =
						content.media?.filter((m) => m.imageType === "screenshot") || [];
	...
	const totalStats = useMemo(() => {
		...
		artifacts.forEach((artifact) => {
			artifact.contents.forEach((content) => {
				if (content.media) {
					stats.totalImages += content.media.filter(
	...
	const estimatedCosts = useMemo(() => {
		...
		Object.entries(MODEL_PRICING).forEach(([key, pricing]) => {
			...
			const outputCost =
				(totalStats.totalTokens / 1_000_000) * pricing.output * 0.5;
			costs.push({
				model: key,
				name: pricing.name,
				inputCost,
