...
const CHART_FONT_FAMILY = '"JetBrains Mono", ui-monospace, SFMono-Regular, Menlo, Monaco, monospace'
...
export function ArtifactCharts({ artifacts, chunkingSettings }: ArtifactChartsProps) {
    ...
    artifacts.forEach(artifact => {
      artifact.contents.forEach(content => {
        if (content.media) {
          ...
          stats.totalScreenshots += content.media.filter(m => m.imageType === 'screenshot').length
        }
      })
    })
  ...
  const pieData = useMemo(() => {
    return [
      ...
      { name: 'Screenshots', value: totalStats.screenshotTokens, color: COLORS.screenshots }
    ].filter(d => d.value > 0)
  }, [totalStats])
  
  const estimatedCosts = useMemo(() => {
    const costs: Array<{ model: string; name: string; inputCost: number; outputCost: number; totalCost: number }> = []
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <Card className="bg-[#ede5d8] border-[#d4c8b8]">
          <CardHeader className="pb-1 pt-3">
            <CardTitle className="text-xs font-bold tracking-tight text-[#2d1b0e]" style={{ fontFamily: CHART_FONT_FAMILY }}>
          ...
          <CardContent className="pt-0 pb-3">
            <ResponsiveContainer width="100%" height={180}>
              <ComposedChart data={pageData} barCategoryGap="15%">
                ...
                <XAxis 
                  dataKey="page" 
                  tick={{ fontSize: 9, fill: '#7a5c3a', fontFamily: CHART_FONT_FAMILY }}
                  tickLine={false}
                ...
                />
                <Bar dataKey="textTokens" stackId="a" fill={COLORS.text} name="Text" radius={[0, 0, 0, 0]} />
                <Bar dataKey="imageTokens" stackId="a" fill={COLORS.images} name="Images" radius={[0, 0, 0, 0]} />
                <Bar dataKey="screenshotTokens" stackId="a" fill={COLORS.screenshots} name="Screenshots" radius={[0, 0, 0, 0]} />
              </ComposedChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
      ...
      <Card className="bg-[#ede5d8] border-[#d4c8b8]">
        ...
        <CardContent className="pt-0 pb-3">
          <div className="grid grid-cols-2 gap-x-8 gap-y-1.5" style={{ fontFamily: CHART_FONT_FAMILY }}>
            ...
            <div className="flex justify-between">
              ...
              <span className="text-[10px] font-bold text-[#2d1b0e]">{totalStats.totalSections}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[10px] text-[#7a5c3a]">Total Tokens</span>
              <span className="text-[10px] font-bold text-[#2d1b0e]">{totalStats.totalTokens.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[10px] text-[#7a5c3a]">Text Tokens</span>
              <span className="text-[10px] font-bold text-[#2d1b0e]">{totalStats.textTokens.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[10px] text-[#7a5c3a]">Image Tokens</span>
              <span className="text-[10px] font-bold text-[#2d1b0e]">{totalStats.imageTokens.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[10px] text-[#7a5c3a]">Screenshot Tokens</span>
              <span className="text-[10px] font-bold text-[#2d1b0e]">{totalStats.screenshotTokens.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-[10px] font-bold text-[#2d1b0e]">{totalStats.totalScreenshots}</span>
            </div>
          </div>
          
          <div className="mt-3 pt-3 border-t border-[#d4c8b8]">
            <div className="text-[10px] font-bold text-[#2d1b0e] mb-2" style={{ fontFamily: CHART_FONT_FAMILY }}>ESTIMATED COSTS (per extraction)</div>
