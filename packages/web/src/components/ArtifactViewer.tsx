	...
	for (let aIdx = 0; aIdx < artifacts.length; aIdx++) {
		...
		for (let cIdx = 0; cIdx < artifact.contents.length; cIdx++) {
			...
			const textTokens = content.text
				? estimateTokens(content.text) * settings.textRatio
				: 0;
			const imgTokens = content.media
				? calculateImageTokens(
						content.media,
						settings.imageTokens,
						settings.filterEmbedded,
						settings.filterScreenshot,
					)
				: 0;
			...
			const wouldExceedTokens =
				currentTokens + contentTokens > settings.maxTokens;
			const imageCount =
				content.media?.filter((m) => {
					if (m.type !== "image") return false;
					if (!settings.filterEmbedded && m.imageType === "embedded")
						return false;
					if (!settings.filterScreenshot && m.imageType === "screenshot")
						return false;
					return true;
				}).length || 0;
			const wouldExceedImages =
				settings.maxImages !== null && imageCount > settings.maxImages;
		...
		artifacts.forEach((artifact, aIdx) => {
			artifact.contents.forEach((content, cIdx) => {
				if (content.page !== undefined) {
					...
					const chunk =
						chunks.find(
							(c) => c.artifactIdx === aIdx && c.contentIdx <= cIdx,
						) ||
						chunks[chunks.length - 1] ||
						null;

					// Get screenshot URL
					const screenshot = content.media?.find(
				...
				return (
					...
					>
						...
						<div
							...
							style={{
								backgroundColor: hasValidChunk
									? getChunkColor(chunkGroup.chunkId)
									: "transparent",
							}}
						>
...
export function ArtifactViewer({
	...
	const [viewMode, setViewMode] = useState<"visual" | "json" | "graphs">(
		"visual",
	);
	...
	const renderVisualView = () => {
		return (
			<div className="flex gap-4">
				...
				{/* Main Content */}
				<div ref={scrollContainerRef} className="flex-1 space-y-3">
					{/* Info Bar with Expand/Collapse - Shows Chunk/Page Info */}
					<div className="flex items-center gap-3 p-3 bg-[#ede5d8] border border-[#d4c8b8] rounded-lg">
						...
						<div className="ml-auto flex items-center gap-2">
							...
							<span className="text-sm text-[#a0926f] ml-2">
					...
					<div className="space-y-3">
						{artifacts.map((artifact, aIdx) => {
							const artifactTokens = artifact.contents.reduce(
								(sum, content) => {
									const textTokens = content.text
									...
									const imgTokens = content.media
										? calculateImageTokens(
												content.media,
												chunkingSettings.imageTokens,
												chunkingSettings.filterEmbedded,
												chunkingSettings.filterScreenshot,
											)
										: 0;
							...
							return (
								<Card
									key={artifact.id}
									className="bg-[#ede5d8] border-[#d4c8b8] flex flex-col gap-3 py-4 px-4"
								>
									<CardHeader className="pb-0 pt-0 px-0">
										<div className="flex items-center justify-between">
											<div className="min-w-0">
												...
												<p className="text-xs text-[#a0926f] truncate mt-0.5">
													{artifact.id}
												</p>
											</div>
											<div className="text-right flex-shrink-0 ml-4">
												...
												<div className="text-sm font-semibold text-[#2d1b0e]">
													{artifactTokens.toLocaleString()}
												</div>
											</div>
										</div>
									</CardHeader>
									<CardContent className="space-y-2 pt-0 pb-0 px-0">
										{artifact.contents.map((content, cIdx) => {
											...
											const isChunkBoundary =
												contentChunk &&
												cIdx > 0 &&
												getChunkForContent(aIdx, cIdx - 1)?.id !==
													contentChunk.id;

											// Calculate individual content tokens (for this page only)
											const contentTextTokens = content.text
												? estimateTokens(content.text) *
													chunkingSettings.textRatio
												: 0;
											const contentImgTokens = content.media
												...
												: 0;
											const contentTotalTokens = Math.round(
												contentTextTokens + contentImgTokens,
											);
											...
											return (
												<div key={contentKey} className="relative">
														...
														<div className="relative py-3 my-1">
															...
															<div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-2 bg-[#ede5d8] px-3 py-1">
																...
																<span
																	...
																	style={{
																		color: getChunkColor(contentChunk.id),
																	}}
																>
													...
													>
															...
															<div className="flex-1 min-w-0">
																...
																<div className="flex items-center gap-2 mb-1">
																	...
																	<div className="ml-auto text-xs text-[#a0926f]">
																		~{contentTotalTokens.toLocaleString()}{" "}
																		tokens
																	</div>
																</div>
														...
														{content.media?.some(
															(m) => m.imageType === "embedded",
														) && (
															<div className="mt-2 pt-2 border-t border-[#d4c8b8]">
																<div className="text-xs text-[#a0926f] mb-1">
																	Embedded Images
																</div>
				...
				{isParsing ? (
					<>
						<div className="w-16 h-16 rounded-full bg-[#ede5d8] flex items-center justify-center mb-4">
							<Loader2 className="h-8 w-8 animate-spin text-[#7a5c3a]" />
						</div>
						<p className="text-sm font-medium text-[#7a5c3a]">
							Parsing files...
						</p>
						<p className="text-xs mt-1 opacity-70">
							Extracting text and images from your documents
						</p>
					</>
				) : (
					<>
						...
						<p className="text-sm font-medium">No artifacts to display</p>
						<p className="text-xs mt-1 opacity-70">
							Upload files and click Parse to see artifacts
						</p>
					</>
				)}
			</div>
	...
	return (
		<>
			<Tabs
				value={viewMode}
				onValueChange={(v) => setViewMode(v as "visual" | "json" | "graphs")}
			>
				...
				<TabsContent value="visual" className="mt-0 relative">
					<div
						className={`transition-opacity duration-300 ${isParsing ? "opacity-50 pointer-events-none" : "opacity-100"}`}
					>
						{renderVisualView()}
					</div>
