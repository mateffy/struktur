...
export function Logo({ className = "", isDesktop = false }: LogoProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex items-center gap-3 w-full ${isDesktop ? "pt-7 pl-3 pr-4 pb-0.5" : "pl-2 pr-4 py-3 pt-3"}`}
      >
        {/* Icon container */}
          ...
          {quadrants.map((quad) => (
            <div
              ...
              className={`absolute inset-0 w-full h-full overflow-hidden pointer-events-none transition-opacity duration-[120ms] ease-out ${
                loadedQuadrants.includes(quadrants.indexOf(quad)) ? "opacity-100" : "opacity-0"
              }`}
              style={{
        ...
        {/* Text */}
        {isDesktop ? (
          <h1 className="text-lg font-semibold text-[#2d1b0e] tracking-tight">struktur</h1>
        ) : (
          <div className="flex flex-col">
            <div className="text-xs text-[#a0926f] italic leading-none">/ʃtʁʊkˈtuːɐ̯/</div>
            <h1 className="text-xl font-semibold text-[#2d1b0e] tracking-tight">struktur</h1>
          </div>
        )}
      </div>
    </a>
