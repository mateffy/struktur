...
import type { ExecutionStatus, ExtractionResult, ProgressInfo } from "./ExtractPage";
...
export function OutputView({
      ...
      <Card>
        <CardContent className="py-16 text-center">
          <p className="text-muted-foreground">
            No output yet. Upload files and run an extraction to see results here.
          </p>
        </CardContent>
      </Card>
    );
    ...
    if (status === "extracting") {
      return (
        <div className="space-y-3">
          ...
          {progress.step && <p className="text-sm text-muted-foreground">{progress.step}</p>}
    ...
    return (
      <div className="space-y-3">
        <div className="flex justify-end">
          <Button variant="outline" size="sm" onClick={() => navigator.clipboard.writeText(json)}>
            Copy
          </Button>
        </div>
  ...
  const renderFormattedContent = () => {
    if (!result?.data) {
      return <p className="text-muted-foreground">No extraction result available</p>;
    }
