		...
		>
			<div className="flex items-center gap-2 min-w-[120px] flex-shrink-0 pt-0.5">
				<span className="text-[#7a5c3a]/70">
					{FieldIcons[type] ?? FieldIcons.string}
				</span>
				<span className="text-sm font-medium text-[#2d1b0e]">
					{name}
				</span>
				{required && (
					<span className="text-[#a05c5c] text-xs">*</span>
				)}
			</div>
			<div className="flex-1 min-w-0">
				<ValueDisplay schema={schema} value={value} />
				{schema.description && (
					<p className="text-xs text-[#7a5c3a]/60 mt-1 leading-relaxed">
						{schema.description}
					</p>
				)}
			</div>
...
function ValueDisplay({
	...
	if (value === null || value === undefined) {
		return (
			<span className="text-sm text-[#a0926f] italic">
				null
			</span>
		);
	}
...
function CollapsibleSection({
	...
	const hasValue = value !== undefined && value !== null;
	const isEmpty = count === 0 || (Array.isArray(value) && value.length === 0) || 
		...
		<div className="border border-[#d4c8b8] rounded-lg overflow-hidden bg-[#f5efe6]">
			...
			>
				...
				<div className="flex items-center gap-2 flex-1">
					{isEmpty ? (
						<span className="text-xs text-[#a0926f] italic">
							Empty
						</span>
					) : (
			...
			{isOpen && (
				<div className="border-t border-[#d4c8b8] bg-[#faf8f3]">
					{children}
				</div>
			)}
		</div>
...
function ObjectViewer({
	...
	if (entries.length === 0) {
		return (
			<div className="p-4 text-center text-sm text-[#a0926f] italic">
				No data
			</div>
		);
	}
			...
			{data.map((item, index) => {
				...
				return (
					<div
						...
						style={{ marginLeft: `${depth * 12}px` }}
					>
						<span className="text-xs text-[#a0926f] font-mono w-6">
							{index + 1}
						</span>
	...
	if (type === "object" && typeof data === "object" && data !== null && !Array.isArray(data)) {
		return (
			<div className={cn("space-y-3", className)}>
				{schema.title && (
					<div className="px-1">
						<h3 className="text-lg font-semibold text-[#2d1b0e]">
							{schema.title}
						</h3>
						{schema.description && (
