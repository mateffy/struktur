import { AlertCircle, Eye, EyeOff, KeyRound, Lock, Shield, Trash2 } from "lucide-react";
      ...
      <DialogContent className="max-w-md bg-[#f5efe6] border-[#d4c8b8] p-0 overflow-hidden">
        <DialogHeader className="bg-gradient-to-br from-[#7a5c3a] to-[#5c452a] p-6 text-white">
          <div className="flex items-center justify-center mb-4">
            <div className="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center backdrop-blur-sm">
              {mode === "setup" ? <KeyRound className="w-8 h-8" /> : <Lock className="w-8 h-8" />}
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-[#2d1b0e]">
                  ...
                  <p className="text-[#7a5c3a] leading-relaxed">
                    Your API keys are encrypted in your browser. The password is never sent to our
                    servers. If you forget it, you will need to re-enter all API keys.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-2">
            <Label htmlFor="password" className="text-sm font-medium text-[#2d1b0e]">
              Password
            </Label>
          ...
          {mode === "setup" && (
            <div className="space-y-2">
              <Label htmlFor="confirmPassword" className="text-sm font-medium text-[#2d1b0e]">
                Confirm Password
              </Label>
              <Input
                id="confirmPassword"
              ...
              {!showResetConfirm ? (
                ...
                <div className="bg-[#f5e6e6] border border-[#c4a8a8] rounded-lg p-3">
                  <p className="text-sm text-[#8a4a4a] mb-3">
                    <strong>Warning:</strong> This will delete all stored API keys. This action
                    cannot be undone.
                  </p>
          ...
          {mode === "setup" && (
            <p className="text-xs text-[#a0926f] text-center">
              This password cannot be recovered. Make sure to remember it or store it safely.
            </p>
          )}

          {/* Footer note */}
          <p className="text-center text-[#a0926f] text-xs pt-2">
            Your API keys are encrypted with your password and are stored locally on your device
            only.
          </p>
        </form>
      </DialogContent>
