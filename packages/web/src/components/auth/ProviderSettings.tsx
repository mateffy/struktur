...
import { Lock, Shield, ExternalLink, AlertCircle, Check, Eye, EyeOff, Trash2, KeyRound } from 'lucide-react'
...
export function ProviderSettings({
  ...
  const handleSave = useCallback(
    async (provider: ProviderId) => {
      ...
      if (!validation.valid) {
        setErrors((prev) => ({ ...prev, [provider]: validation.error || 'Invalid key' }))
        return
      }
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-[#7a5c3a] leading-relaxed">
                      API keys are encrypted in your browser's LocalStorage and
                      only decrypted when needed. They are sent directly to the
                      model provider when you run extractions and are never stored
                      on our servers. We recommend setting billing limits and
