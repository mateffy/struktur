...
export function SecureStorageGate({ children }: SecureStorageGateProps) {
  const { isInitialized, isUnlocked, initialize, unlock, reset, error, status, unlockRequested, clearUnlockRequest } = useApiKeys()
  ...
  const handleSubmit = useCallback(
    async (password: string) => {
      ...
      try {
        ...
        // Error is already handled by setting localError
        throw err
      }
    },
