...
export type ProviderId = "openai" | "anthropic" | "google" | "opencode" | "openrouter";
 ...
 */
export async function initializeSecureStorage(password: string): Promise<void> {
  ...
  localStorage.setItem(MASTER_KEY_STORAGE_KEY, JSON.stringify(encryptedMasterKey));
 ...
 */
export async function storeApiKey(provider: ProviderId, apiKey: string): Promise<boolean> {
  ...
  try {
    ...
    localStorage.setItem(`${API_KEY_PREFIX}${provider}`, JSON.stringify(storedData));
 ...
 * Requires an active session
 */
export async function retrieveApiKey(provider: ProviderId): Promise<string | null> {
  const masterKey = await getMasterKey();
...
export function getStoredProviders(): ProviderId[] {
  ...
  const allProviders: ProviderId[] = ["openai", "anthropic", "google", "opencode", "openrouter"];
...
export async function changeStoragePassword(
  ...
  try {
    ...
    localStorage.setItem(MASTER_KEY_STORAGE_KEY, JSON.stringify(newEncryptedMasterKey));
