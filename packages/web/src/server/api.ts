...
import type { NpmParserDef, ParsersConfig, SerializedArtifact } from "@struktur/sdk";
import {
  agent,
  detectMimeType,
  doublePass,
...
const CONFIG_DIR =
  process.env.STRUKTUR_CONFIG_DIR ?? path.join(os.homedir(), ".config", "struktur");
...
export async function parseFiles(
  ...
  for (const file of files) {
    ...
    if (parserDef) {
      const parsed = await runParser(parserDef, { kind: "buffer", buffer }, mimeType);
      ...
      fileArtifacts = [
        {
          id: `artifact-${crypto.randomUUID()}`,
          type: "image",
          contents: [{ media: [{ type: "image", base64: buffer.toString("base64") }] }],
        },
      ];
    } else {
...
export async function extractData(
  ...
  try {
    ...
    const resolvedModel = resolvedModelSpec || (await getDefaultModel()) || "openai/gpt-4o-mini";
