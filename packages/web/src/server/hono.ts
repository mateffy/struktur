...
import { parseFiles, extractData, saveExtraction, generateExtractionId, type ExtractionEvent } from './api'
import { fetchModels, fetchConfig } from './models'
...
app.post('/api/extract/stream', async (c) => {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        sendEvent({ type: 'step', data: { step: 0, total: 1, label: 'parsing' } })
        const artifacts = await parseFiles(files, parsingOptions ?? {})
        sendEvent({ type: 'step', data: { step: 1, total: 1, label: 'parsed', data: { artifactCount: artifacts.length } } })
        ...
        sendEvent({ type: 'error', data: { message: (error as Error).message } })
      } finally {
        controller.close()
      }
    },
